gfmc_real <- function(temp, rh, ws, prec, isol, GFMCold = 85, batch = TRUE, time.step = 1, roFL = 0.3,
                 out = "GFMCandMC") {
    #############################################################################
    ...
    if (out=="ALL"){
        return(c(temp, rh, ws, prec, isol, GFMC, MC))
    } else if(out == "GFMC"){
        return(GFMC)
    } else if (out == "MC"){
        return(MC)
...
gfmc <- function(input, GFMCold = 85, batch = TRUE, time.step = 1, roFL = 0.3,
                 out = "GFMCandMC") {
  r <- gfmc_real(input$temp, input$rh, input$ws, input$prec, input$isol, GFMCold, batch, time.step, roFL, out)
