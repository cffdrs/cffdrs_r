...
fireSeason <- function(
  ...
  if ("day" %in% names(input)) {
    ...
    stop(paste0(
      "day was not provided, day is required for this function,",
      " stopping execution.."
    ))
  }
  ...
  if ("tmax" %in% names(input)) {
    ...
    stop(paste0(
      "Maximum Daily Temperature (tmax) was not provided, daily tmax is",
  ...
  if (method != "wf93" & method != "la08") {
    stop(paste0(
      ...
      "\" is unavailable, read documentation for available methods."
    ))
  }
  ...
  if (method == "wf93") {
    ...
    if (method == "la08") {
      # In regions normally covered by snow, begin calculations 3rd day after
      # snow has left the area
      if (consistent.snow) {
        # Check that data has snow_depth variable
        if (!("snow_depth" %in% names(input))) {
          stop(paste0(
            "Snow depth is required for the selected method \"", method,
            "\", read documentation for appropriate use."
          ))
        } else {
          ...
          for (k in 1:n0) {
            ...
            if (k > 3) {
              # If we are not already in a fire season AND
              #  the last 3 days (inclusive) are all snow free
              # then set the starting date
