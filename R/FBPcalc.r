.FBPcalc  <- function(input=NULL,output="Primary"){                                                                                           # The choice of output include: "Primarry" (default), "Secondary", and "All".
  ...
  if (is.null(input)) {
    ...
    if (!exists("FUELTYPE") | is.null(FUELTYPE)){ 
      warning("FuelType is a required input, default FuelType = C2 is used in the calculation")
      FUELTYPE <- rep("C2", n0)}
    if (!exists("FFMC") | is.null(FFMC)){ 
      warning("FFMC is a required input, default FFMC = 90 is used in the calculation")
      FFMC <- rep(90, n0)}
  ...
  if (output == "SECONDARY" | output == "ALL" | output == "S" | 
    ...
    LBt <- ifelse(ACCEL == 0, LB, .LBtcalc(FUELTYPE, LB, 
                                           HR, CFB))
    BROS <- .BROScalc(FUELTYPE, FFMC, BUI, WSV, FMC, SFC, 
                      PC, PDF, CC, CBH)
    FROS <- .FROScalc(ROS, BROS, LB)
    E <- sqrt(1 - 1/LB/LB)
    TROS <- ROS * (1 - E)/(1 - E * cos(THETA - RAZ))
    ROSt <- ifelse(ACCEL == 0, ROS, .ROStcalc(FUELTYPE, ROS, 
                                              HR, CFB))
    BROSt <- ifelse(ACCEL == 0, BROS, .ROStcalc(FUELTYPE, 
                                                BROS, HR, CFB))
    FROSt <- ifelse(ACCEL == 0, FROS, .FROScalc(ROSt, BROSt, 
                                                LBt))
    ...
    DH <- ifelse(ACCEL == 1, .DISTtcalc(FUELTYPE, ROS, HR, 
          CFB), ROS * HR)
    DB <- ifelse(ACCEL == 1, .DISTtcalc(FUELTYPE, BROS, HR, 
          CFB), BROS * HR)
