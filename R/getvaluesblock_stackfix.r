...
#' back to it.
#' 
#' @importFrom raster filename inMemory getValuesBlock brick stack nlayers
#'
  ...
  if (class(x) == "RasterStack") {
    ...
    unique_getValuesBlock <- foreach(single_filename = unique_filenames, 
                                     .packages = c("raster")) %dopar% {
                                       if (single_filename != "") 
                                         raster::getValuesBlock(raster::brick(single_filename), row, nrows, 
                                                        col, ncols)
                                       else raster::getValuesBlock(raster::stack(x, bands = inMemory_layers), 
                                                           row, nrows, col, ncols)
                                     }
    ...
    if (class(x) == "RasterLayer" || nlayers(x) == 
        1) 
      return(raster::getValuesBlock(x, row = row, nrows = nrows, 
                            col = col, ncols = ncols))
    else return(raster::getValuesBlock(x, row = row, nrows = nrows, 
