...
#'
#' @description \code{pros} is used to calculate the rate of spread and
...
#' decisecond)\cr
#' \var{Long1}
#'    \tab (required) \tab Longitude for datalogger 1. (decimal degrees). \cr
#' \var{Long2}
#'    \tab (required) \tab Longitude for datalogger 2. (decimal degrees). \cr
#' \var{Long3}
#'    \tab (required) \tab Longitude for datalogger 3. (decimal degrees). \cr
#' \var{Lat1}
#'    \tab (required) \tab Latitude for datalogger 1. (decimal degrees). \cr
#' \var{Lat2}
#'    \tab (required) \tab Latitude for datalogger 2. (decimal degrees). \cr
#' \var{Lat3}
#'    \tab (required) \tab Latitude for datalogger 3. (decimal degrees). \cr }
...
pros <- function(input) {
  ...
  if (any(exist)) {
    errmsg <- paste(
      "cffdrs::pros Column ", colnames(input[exist[TRUE]]),
      " is required in column list."
    )
    stop(errmsg)
  }
  LengthT1T2 <- geosphere::distHaversine(
    cbind(input$LONG1, input$LAT1),
    cbind(input$LONG2, input$LAT2)
  )
  LengthT1T3 <- geosphere::distHaversine(
    cbind(input$LONG1, input$LAT1),
    cbind(input$LONG3, input$LAT3)
  )
  LengthT2T3 <- geosphere::distHaversine(
    cbind(input$LONG2, input$LAT2),
    cbind(input$LONG3, input$LAT3)
  )
  bearingT1T2 <- geosphere::bearing(
    cbind(input$LONG1, input$LAT1),
    cbind(input$LONG2, input$LAT2)
  )
  bearingT1T3 <- geosphere::bearing(
    cbind(input$LONG1, input$LAT1),
    cbind(input$LONG3, input$LAT3)
  )
  bearingT2T3 <- geosphere::bearing(
    cbind(input$LONG2, input$LAT2),
    cbind(input$LONG3, input$LAT3)
  )
  RefT1T2bearing <- geosphere::bearing(
    cbind(input$LONG1, input$LAT1),
    cbind(input$LONG2, input$LAT2)
  )
  AngleArad <- acos(
    (LengthT1T3^2 + LengthT1T2^2 - LengthT2T3^2)
